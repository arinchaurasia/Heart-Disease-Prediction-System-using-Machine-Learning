...
""")
    ...
    if uploaded_file is not None:
        ...
        # Load all models
        dt_model  = pickle.load(open('tree.pkl', 'rb'))
        ...
        if set(expected_columns).issubset(input_data.columns):
            ...
            input_data['Prediction DT']  = ''
            input_data['Prediction LR']  = ''
            input_data['Prediction RF']  = ''
            input_data['Prediction SVM'] = ''

            # Row-wise prediction
            for i in range(len(input_data)):
                row = input_data.iloc[i][expected_columns].values.reshape(1, -1)

                input_data.at[i, 'Prediction DT']  = dt_model.predict(row)[0]
                input_data.at[i, 'Prediction LR']  = lr_model.predict(row)[0]
                input_data.at[i, 'Prediction RF']  = rf_model.predict(row)[0]
                input_data.at[i, 'Prediction SVM'] = svm_model.predict(row)[0]

            # Save CSV
            input_data.to_csv('PredictedHeartAllModels.csv', index=False)
...
with tab3:
    ...
    data = {'Decision Tree' : 80.97, 'Logistic Regression' : 85.86, 'Random Forest' : 84.23, 'Support Vector Machine' : 84.23}
